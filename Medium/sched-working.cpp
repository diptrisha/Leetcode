...
class Event
{
    public:
        ...
        State eventState;
        Event(int timestamp, Process *proc, TransitionState tran)// int eventID=0)
        {
           // eventId = eventID;
...
void simulation(){
  ...
  while(event = mydes.get_event()){
    ...
    if(call_sched){
      if(!mydes.eventQ.empty() && mydes.eventQ.front()->eventTimestamp == current_time){
        continue;
      }
      call_sched = false;
          ...
          case 's':
            ...
            for (int i=0;i<schedulingAlgo.length();i++)
            {
               ...
               if (schedulingAlgo.at(i)=='E')
              {
                ...
                //printf("quantum:%d \n", quantum);
                //printf("max prio:%d \n", maxPrio);
                cout<<"PREPRIO"<<" "<<to_string(quantum)<<"  "<<to_string(maxPrio)<<endl;
                sched= new PREPRIO();
